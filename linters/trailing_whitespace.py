...
# taken from pre-commit hooks

import os
import argparse


def process_file(filename, is_markdown, chars):
...
def main():
    ...
    for ext in md_exts:
        if any(c in ext[1:] for c in r'./\:'):
            parser.error(
                'bad --markdown-linebreak-ext extension '
                '{ext!r} (has . / \\ :)\n'
                "  (probably filename; use '--markdown-linebreak-ext=EXT')".format(ext=ext),
            )
    ...
    for filename in args.filenames:
        ...
        if process_file(filename, md, chars):
